...
revenue = pd.Series(np.random.normal(6000, 300, 24), index=dates)
...
plt.figure()
plt.plot(revenue / 1e6, label="Historical Revenue (in millions)")
plt.plot(forecast_series / 1e6, label="Forecast (in millions)", linestyle="--")
plt.legend()
plt.title("Revenue Forecast (in millions)")
st.ylabel("USD (Millions)")
st.pyplot(plt)
...
plt.figure()
plt.plot(op_income / 1e6, label="Operating Income (in millions)")
plt.title("Operating Income Trend (in millions)")
